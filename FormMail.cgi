...
BEGIN
{
  ...
  $postmaster        = '';
  @referers          = qw(jdl.people.uic.edu localhost);
  @allow_mail_to     = qw(jdl@uic.edu localhost);
  @recipients        = ();
  %recipient_alias   = ();
...
sub spam {
    my $q = new CGI;
    my $spamcheck = $q->param('spamcheck') || '';
    if ($spamcheck ne '') {
	print "Location: http://jdl.people.uic.edu/spamfail.htm\n\n";
	exit;
    }
    my $optionarg = $q->param('optional') || '';
    if ($optionarg ne '') {
	print "Location: http://jdl.people.uic.edu/spamfail.htm\n\n";
	exit;
    }
